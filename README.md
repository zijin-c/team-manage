...
- **Team 账号管理**
  ...
  - **授权登录链接**：一键生成引导步骤，在本地浏览器完成登录后将 Session JSON 或 Access Token 回传，自动填入账号
  - **账号专用代理**：可为每个 Team 账号单独配置代理，优先级高于全局代理
...
- **全局高斯模糊背景**：所有页面（管理员面板、用户兑换页、登录页）均应用高强度高斯模糊背景图，视觉效果一致统一
...
3. **通过授权登录链接更新 Token**
   - 进入 Team 列表，点击"编辑"
   - 点击"生成授权登录链接"按钮
   - 按步骤在本地浏览器登录 ChatGPT，获取 Session JSON
   - 将 JSON 粘贴回对话框，点击"应用 Token"即可自动填入
